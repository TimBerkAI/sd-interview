...
import { Plus, Trash2, ArrowLeft, Check, BookOpen } from 'lucide-react';
...
export function ArchTasksPage() {
  ...
  const createNew = async () => {
    const task = await api.createArchTask({ name: 'New Task', status: TaskStatus.DRAFT });
    if (task) navigate(`/admin/arch/tasks/${task.id}`);
  };
      ...
      {tasks.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {tasks.map((task) => (
            <div
              key={task.id}
              onClick={() => navigate(`/admin/arch/tasks/${task.id}`)}
            ...
            >
              ...
              <div className="flex items-center justify-between mt-auto pt-2">
                ...
                <span className="text-xs text-gray-300 dark:text-gray-700">#{task.id}</span>
              </div>
            </div>
          ))}
        </div>
...
export function ArchTaskEditPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [form, setForm] = useState({ name: '', description: '', status: TaskStatus.DRAFT });
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (!id) return;
    api.getArchTask(Number(id)).then((task) => {
      if (task) setForm({ name: task.name, description: task.description || '', status: task.status });
      setLoading(false);
    });
  }, [id]);
  ...
  const save = async () => {
    if (!id) return;
    setSaving(true);
    await api.updateArchTask(Number(id), form);
    setSaving(false);
  };
  ...
  return (
    <div className="p-8 max-w-3xl">
      <button
        onClick={() => navigate('/admin/arch/tasks')}
